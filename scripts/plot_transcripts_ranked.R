...
png(str_c(outpath, "/", prefix, "_transcriptsRanked_withSd.png"), bg="transparent",units="in",width = 4.25, height= 3.75 ,res=600)
...
ggplot(data=x.ranked, aes(x=isoform_index, fill = Isoform_class)) +
  ...
  scale_x_continuous(name = "Transcripts ranked by expression (NE)") +
...
png(str_c(outpath, "/", prefix, "_transcriptsRanked.png"), bg="transparent",units="in",width = 4.25, height= 3.75 ,res=600)

ggplot(data=x.ranked, aes(x=isoform_index, fill = Isoform_class)) +
  geom_bar(aes(y = NE), color=NA, size=0.3, width=0.8, stat="identity") +
  scale_fill_brewer(palette = "Dark2") +
  scale_y_continuous(name = "Transcript expression\nacross samples; NE") +
  scale_x_continuous(name = "Transcripts ranked by expression (NE)") +
  guides(fill=guide_legend(title="Transcript category")) +
